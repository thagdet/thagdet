...
#if defined(NDEBUG)
# error "Detcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x5d84eb4607346716bf962cbfdcc00204c68cf139f32ba0964224b86ad90860f8");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Detcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Detcoin Signed Message:\n";
...
{
    // Detcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 50); // Detcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 600; // Detcoin: 10mins
static const int64 nTargetSpacing = 30; // Detcoin: 30sec
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Detcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Detcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x5d84eb4607346716bf962cbfdcc00204c68cf139f32ba0964224b86ad90860f8");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nVersion = 1;
        block.nTime    = 1508919892;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1353939;
        ...
        assert(block.hashMerkleRoot == uint256("0x17fa80b3bbc750360dc5fee1e526b10f176be3bb25467f4a4faf53c90738af21"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Detcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("DetcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("DetcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static DetcoinMiner(CWallet *pwallet)
{
    printf("DetcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("detcoin-miner");
    ...
    try { loop {
        ...
        printf("Running DetcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("DetcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&DetcoinMiner, pwallet));
}
